...
function BedRequestDetails() {
     const [allTenantsDetailsArray,setAllTenantsDetailsArray]=useState([]);
      useEffect(()=>{
        const getTenantsDetailsArrayFunction=async()=>{
          try {
            const res=await fetch("https://peace-pg-s-management-system.onrender.com/api/get/bedRequestDetails",{
              method:"GET",
              headers:{
            ...
            const updatedArray=data.bedsRequestDetailsArray;
    ...
    try {
        const res=await fetch(`https://peace-pg-s-management-system.onrender.com/api/delete/bedRequest/${id}`,{
           method:"DELETE",
           headers:{
            "Content-Type":"application/json"
           },
  ...
  return (
   ...
   {allTenantsDetailsArray.length===0
       ?
       <section className='flex justify-center items-center w-full'>
        <h2 className='lora text-center tracking-[3px] text-[40px] text-red-500'>OOP's Currently No Beds Request</h2>
       </section>
       :
    ...
    grid-cols-1 md:grid-cols-2 gap-10 p-5  py-10 items-center justify-items-center '>
    
      {
        ...
        allTenantsDetailsArray?.map((tenant,idx)=>{
          return(
             <section>
                <h2 className='text-center bg-gradient-to-r lora tracking-[3px] text-[#f09b9b] from-[#360c6561] to-[#00b5f269] 
       py-10 font-black text-6xl'>All Bed Request Form Details</h2>
...
export default BedRequestDetails